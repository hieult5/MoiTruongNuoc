...
namespace MTN.Common
{
    ...
    {
        public static Dictionary<string, string[,]> GetDataSearch(this DbEntities db, string fromDate, string toDate, string isDuBao, string diadanhid)
        {
            // fix data request
            ...
            string[,] arr;
            var maubaocao = db.NV_MauBaocao.Where(x => x.Diadanh_ID.Equals(diadanhid) && x.Trangthai).Select(x => x.MauBC_ID).FirstOrDefault();
            if (string.IsNullOrEmpty(maubaocao))
                return list;
            var lstTT = db.NV_MaubaocaoThuoctinh.Where(x => x.MauBC_ID.Equals(maubaocao)).OrderBy(x => x.STT).Select(row => row.Thuoctinh_ID);
            var lstDD = db.NV_MaubaocaoDiadanh.Where(x => x.MauBC_ID.Equals(maubaocao)).OrderBy(x => x.STT).Select(row => row.Diadanh_ID);
