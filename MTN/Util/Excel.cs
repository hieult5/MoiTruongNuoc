...
namespace MTN.Util
{
    public static class Excel
    {
        public static string getCopyExcelTemplateFile(out string newFile)
        {
            ...
            if (!Directory.Exists(Path.GetDirectoryName(templateFile) + "\\temp\\"))
            {
                Directory.CreateDirectory(Path.GetDirectoryName(templateFile) + "\\temp\\"); 
            }
            string tempFile = Path.GetDirectoryName(templateFile) + "\\temp\\" + newFile;
        ...
        public static bool ReadAndWriteDataToExcel(this string filePath, string diadanhid, bool isBC_QuanTrac)
        {
            bool response = true;
                        ...
                        try
                        {
                            workBookPart.Workbook.Descendants<Sheet>().ForEach(sheet =>
                            {
                                if (response)
                                {
                                    ...
                                    bool res = sheetData.WriteDatabase(sheet.Name, workBookPart, db, diadanhid, isBC_QuanTrac);
        ...
        private static bool WriteDatabase(this SheetData sheetData, string sheetName, WorkbookPart workBookPart, Models.DbEntities db, string diadanhid, bool isBC_QuanTrac = true)
        {
            ...
            if (string.IsNullOrEmpty(baocaoId))
                return false;
            var lstTT = db.NV_MaubaocaoThuoctinh.Where(x => x.MauBC_ID.Equals(baocaoId)).OrderBy(x => x.STT).Select(row => row.BaocaoThuoctinh_ID);
            var lstDD = db.NV_MaubaocaoDiadanh.Where(x => x.MauBC_ID.Equals(baocaoId)).OrderBy(x => x.STT).Select(row => row.BaocaoDiadanh_ID);
            ...
            try
            {
                ...
                {
                    var lastEntity = db.NV_Dulieudubao.AsEnumerable()
